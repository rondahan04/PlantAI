...
export async function discoverNurseries(
  ...
  const res = await fetchImpl(PLACES_SEARCH_URL, {
    ...
    body: JSON.stringify({
      ...
      regionCode,
      /*
       * Ask for the full page, not `maxResults`. One Text Search request costs
       * the same whether it returns one place or twenty, and the contact-only
       * rows are drawn from the same page - asking for ten places and then
       * splitting them two ways would halve the scrapable shops we find.
       */
      pageSize: Math.min(20, Math.max(1, maxResults + contactOnlyMax)),
